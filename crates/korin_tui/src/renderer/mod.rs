...
fn render_node(buffer: &mut Buffer, view: &BufferView, runtime: &Runtime, id: NodeId) {
    ...
    let inner_x = u16::from(borders.contains(Borders::LEFT));
    let inner_y = u16::from(borders.contains(Borders::TOP));
    let inner_w = node_view
        .width()
        .saturating_sub(inner_x + u16::from(borders.contains(Borders::RIGHT)));
    let inner_h = node_view
        .height()
        .saturating_sub(inner_y + u16::from(borders.contains(Borders::BOTTOM)));
